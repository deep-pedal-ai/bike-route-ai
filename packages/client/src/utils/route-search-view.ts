...
// line-opacity value for the routes layer given the currently hovered route id
// (already string-normalized by the caller, or null when nothing is hovered).
//   - null      → a uniform FULL_OPACITY (no highlight in play).
//   - '<id>'    → a `case` expression: the hovered route at FULL_OPACITY, every
//     other route dimmed to DIM_OPACITY. The feature side is normalized with
//     `to-string` so a string hovered id matches a numeric feature id.
// Returns a bare number for the uniform case (a valid paint value) and an
// expression otherwise; line-opacity accepts either.

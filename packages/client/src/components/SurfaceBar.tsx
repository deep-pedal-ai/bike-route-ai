...
export default function SurfaceBar({ surfaces }: SurfaceBarProps) {
  if (surfaces.length === 0) {
    return <p className="text-sm text-[var(--color-moss-muted)]">Surface data unavailable</p>;
  }
    ...
    <div className="space-y-3">
      <div className="flex h-2.5 w-full overflow-hidden rounded-full bg-[var(--color-bark-soft)]">
        {surfaces.map((s) => (
          <div
            key={s.label}
            className={`${SURFACE_COLORS[s.label] ?? 'bg-[var(--color-moss-muted)]'} transition-all duration-500`}
      ...
      <div className="flex flex-wrap gap-3">
        {surfaces.map((s) => (
          <div key={s.label} className="flex items-center gap-1.5">
            <div className={`h-2.5 w-2.5 rounded-full ${SURFACE_DOT_COLORS[s.label] ?? 'bg-[var(--color-moss-muted)]'}`} />
            <span className="text-xs text-[var(--color-sage-text)]">
              {s.label}{' '}
              <span className="font-semibold text-[var(--color-ink)]">{s.percentage}%</span>
            </span>
          </div>
        ))}
      </div>
